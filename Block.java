...
public class Block {
    ...
    public synchronized String computeMerkleRoot(ArrayList<String> lstItems) {
        ...
        ArrayList <MerkleNode> HashFunction = new ArrayList<>();
        for(int i=0; i < lstItems.size(); i++){
            MerkleNode oNode = new MerkleNode();
            oNode.sHash = BlockchainUtil.generateHash(lstItems.get(i));
            HashFunction.add(oNode);
        }
        return MerkleHelper(HashFunction);
 ...
 // taking arraylist values and turning them into nodes with hashes
    ...
    */
    ...
    private synchronized String MerkleHelper (ArrayList<MerkleNode> nodes){
        if (nodes.size () == 1){
            return nodes.get(0).sHash;

        }
        else {
            ArrayList <MerkleNode> newNodes = new ArrayList <>();
            int countNodes = 0;
            while(countNodes < nodes.size()-1){
                MerkleNode populatedNode= new MerkleNode();
                populateMerkleNode (populatedNode, nodes.get(countNodes), nodes.get(countNodes+1));
                newNodes.add(populatedNode);
                countNodes += 2;
            }
            return MerkleHelper(newNodes);
        }
    }
    ...
    private void populateMerkleNode(MerkleNode oNode, MerkleNode oLeftNode, MerkleNode oRightNode){
        // 1.This method will work just as we did in class...
        // set the left and right nodes and then call generateHash
        // in the BlockchainUtil class and set the node’s hash.
